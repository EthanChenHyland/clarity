...
async function prepareArchiveTarget(target, temporaryDirectory, destinationDirectory) {
  ...
  if (target.archiveType === 'zip') {
    extractZipArchive(archivePath, extractionDirectory);
  } else {
...
function extractZipArchive(archivePath, extractionDirectory) {
...
function checkoutPinnedSource(target, sourceDirectory, workingDirectory) {
  execFileSync('git', [
    'clone', '--depth', '1', '--branch', `v${WHISPER_CPP_VERSION}`, '--single-branch',
    target.repositoryUrl, sourceDirectory
  ], {
    cwd: workingDirectory,
    env: process.env,
    stdio: 'inherit',
  ...
  if (actualCommit !== target.commit) {
    throw new Error(`whisper.cpp source commit mismatch: expected ${target.commit}, received ${actualCommit}.`);
  }
}
...
async function prepareMacTarget(target, temporaryDirectory, destinationDirectory) {
  ...
  checkoutPinnedSource(target, sourceExtractionDirectory, temporaryDirectory);
  const sourceDirectory = sourceExtractionDirectory;
...
async function prepareWhisperRuntime({
  ...
  if (!(await hasCurrentRuntime(cachedRuntimeDirectory, target))) {
    const temporaryDirectory = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'clarity-whisper-runtime-'));
