...
// stream({ system, turns:[{role,text}], imageDataUrl, maxTokens, onToken, signal, onProgress = () => {} }) -> Promise<fullText>
...
const DEFAULT_MODELS = {
  openai: 'gpt-4o-mini',
  anthropic: CURRENT_ANTHROPIC_FAST,
...
const DEAD_ANTHROPIC_MODEL_RE = /^claude-(?:3|3-5|3-7)(?:-|$)/i;
...
function formatProviderErrorMessage(error, provider, model) {
  ...
  if (isNotFoundError(error)) {
    ...
    return `${label} model${modelHint} is unavailable (404) — it may have been renamed, retired by the provider, or misspelled. Open Settings and pick a current model for ${label} (or clear the field to use Clarity's default), then try again.`;
  }
...
async function streamOpenAI({ apiKey, baseURL, model, system, turns, imageDataUrl, maxTokens, onToken, signal, onProgress = () => {} }) {
...
async function streamAzure({ apiKey, model, system, turns, imageDataUrl, maxTokens, onToken, endpoint, signal, onProgress = () => {} }) {
...
async function streamAnthropic({ apiKey, model, system, turns, imageDataUrl, maxTokens, onToken, signal, onProgress = () => {} }) {
...
async function streamGemini({ apiKey, model, system, turns, imageDataUrl, maxTokens, onToken, signal, onProgress = () => {} }) {
...
async function streamOllama({ apiKey, model, system, turns, imageDataUrl, maxTokens, onToken, signal, onProgress = () => {} }) {
...
function createLLM(settings) {
  ...
  if (provider === 'anthropic' && DEAD_ANTHROPIC_MODEL_RE.test(model || '')) {
    model = tier === 'smart' ? CURRENT_ANTHROPIC_SMART : CURRENT_ANTHROPIC_FAST;
  }
  ...
  // Modern reasoning-capable models can spend a meaningful part of the output
  // allowance before emitting visible text. The previous 700-token fast cap
  // could therefore produce an apparently blank response even on success.
  const maxTokens = settings.smart ? 4000 : 2000;
  ...
  return {
    ...
    async stream(params) {
      ...
      try {
        let result;
        if (provider === 'openai') result = await streamOpenAI(args);
        else if (provider === CUSTOM_PROVIDER) result = await streamOpenAI(args);
        else if (provider === 'ollama') result = await streamOllama(args);
        else if (provider === 'groq') result = await streamOpenAI({ ...args, baseURL: 'https://api.groq.com/openai/v1' });
        else if (provider === 'minimax') result = await streamOpenAI({ ...args, baseURL: MINIMAX_BASE_URLS[minimaxRegion] || MINIMAX_BASE_URLS.global_en });
        else if (provider === 'anthropic') result = await streamAnthropic(args);
        else if (provider === 'gemini') result = await streamGemini(args);
        else if (provider === 'azure') result = await streamAzure(args);
        else throw new Error('unknown provider: ' + provider);
        if (!String(result || '').trim()) {
          throw new Error(`${normalizeProviderName(provider)} returned an empty response from ${model}. Try again or switch to the Smart model.`);
        }
        return result;
      } catch (error) {
