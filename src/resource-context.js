...
function tokenize(value) {
  const matches = String(value || '').toLowerCase().match(/[a-z0-9][a-z0-9_.+#/-]{1,}/g) || [];
...
class ResourceContextIndex {
  ...
  search(query, { limit = 5, maxChars = DEFAULT_CONTEXT_CHARS } = {}) {
    ...
    if (!queryTokens.length || !this.data.chunks.length) return [];
    const ranked = this.data.chunks
    ...
    for (const entry of ranked) {
      ...
      const fileKey = `${entry.chunk.sourceUrl}|${entry.chunk.path}`;
      const fileCount = perFile.get(fileKey) || 0;
      if (fileCount >= 2) continue;
      const remaining = maxChars - usedChars;
      if (remaining < 180) break;
      const text = entry.chunk.text.slice(0, remaining);
      results.push({
        repository: entry.chunk.repository,
