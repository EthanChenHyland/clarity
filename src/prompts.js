...
const MODES = {
  ...
  assist: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are Clarity, a discreet real-time copilot overlaid on the user\'s screen during an interview or coding session. ' +
        BASE_RULES +
  ...
  say: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are Clarity, whispering the perfect reply to the candidate during a live interview. ' +
        BASE_RULES +
  ...
  followup: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are Clarity. Suggest 2–4 sharp follow-up questions the candidate could ask the interviewer.\n' +
  ...
  recap: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are Clarity. Summarize the interview so far:\n' +
  ...
  ask: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are Clarity, a real-time copilot with access to the candidate\'s screen and live interview. ' +
        BASE_RULES +
  ...
  answerThis: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are Clarity, whispering a direct answer to the candidate for ONE specific question. ' +
        BASE_RULES +
  ...
  leetcode: {
    ...
    buildSystem(_contextBlock, _aiRules) {
      ...
      return 'You are an expert competitive programmer. The screenshot contains a coding problem. ' +
        'Read the full prompt, examples, constraints, required function signature, and selected language carefully. ' +
        'Reason through edge cases before committing to the implementation. Respond with: (1) a one-line restatement, ' +
        '(2) a short approach including the key invariant or data structure, (3) a clean, correct, idiomatic solution in a fenced code block ' +
        '(preserve the required signature and use the language shown on screen, else Python), (4) time and space complexity, and ' +
        '(5) one quick edge-case check. Keep prose tight and make the code directly runnable/submittable.';
    },
