...
class LocalWhisperTranscriber {
  ...
  _enqueue(channel, pcm) {
    if (this.discardPendingJobs) return Promise.resolve();
    if (this.pendingJobs >= 8) {
      this.acceptingAudio = false;
      this.discardPendingJobs = true;
      this.onError(new Error('Local transcription cannot keep up. Try a smaller model and restart listening.'));
