// What Iris is allowed to know about Clarity.
//
...
function describeState({ state, transcript, settings, sttDisabled, shortcuts, windowAlive }) {
  ...
  return {
    ...
    // The single most useful field here. When this is true Clarity looks alive and
    ...
    // the user presses the key, nothing happens, and Clarity never knew.
    shortcuts: shortcuts || {},
 ...
 *
 * On the Node transport Clarity cannot read the peer's credentials, so the caller's
...
function consentCopy(request) {
  ...
  return {
    trusted,
    message: action ? `${who} wants to control Clarity.` : `${who} wants to see what Clarity is doing.`,
    detail:
      (action
        ? 'It would be able to start and stop listening. '
        : 'It would be able to read Clarity’s status, recent warnings and errors — never your transcript, your résumé or your API keys. ') +
      (trusted
        ? 'Its code signature has been verified.'
        : 'Clarity cannot verify what this program really is; anything running under your account could make the same claim.') +
      '\n\nYou can change this later in Clarity’s settings.',
