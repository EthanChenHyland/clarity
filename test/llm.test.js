...
test('createLLM: uses current Anthropic defaults when no model is configured', () => {
  assert.equal(createLLM(anthropicSettings({ models: {} })).model, CURRENT_ANTHROPIC_FAST);
  assert.equal(createLLM(anthropicSettings({ models: {}, smart: true })).model, CURRENT_ANTHROPIC_SMART);
});

test('createLLM: self-heals retired Anthropic defaults by tier', () => {
  const models = { anthropic: { fast: 'claude-3-5-haiku-latest', smart: 'claude-3-5-sonnet-latest' } };
  assert.equal(createLLM(anthropicSettings({ models })).model, CURRENT_ANTHROPIC_FAST);
  assert.equal(createLLM(anthropicSettings({ models, smart: true })).model, CURRENT_ANTHROPIC_SMART);
});

test('createLLM: leaves a current user-selected Anthropic model alone', () => {
  const llm = createLLM(anthropicSettings({
    models: { anthropic: { fast: 'claude-opus-4-6', smart: 'claude-opus-4-6' } }
  }));
  assert.equal(llm.model, 'claude-opus-4-6');
});
