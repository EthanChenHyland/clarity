...
test('a stalled local server has a bounded audio backlog and reports recovery', async () => {
  const errors = [];
  const transcriber = new LocalWhisperTranscriber({
    sessionOptions: {},
    sessionFactory: () => ({ start: async () => {}, transcribe: () => new Promise(() => {}), abortInferences() {}, stop: async () => {} }),
    segmenterFactory: options => ({ push: pcm => options.onUtterance(options.channel, pcm), stop() {} }),
    onError: error => errors.push(error.message)
  });
  await transcriber.start();
  for (let i = 0; i < 100; i++) transcriber.push('you', Buffer.from('utterance'));
  assert.ok(transcriber.pendingJobs <= 8);
  assert.equal(errors.length, 1);
  assert.match(errors[0], /smaller model/);
  await transcriber.forceStop();
});
