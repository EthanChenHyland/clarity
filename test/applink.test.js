...
test('reports what Clarity is doing', () => {
...
/**
 * The one test in this file that matters more than the others. Clarity's transcript
...
test('hedges the consent sheet when the caller cannot be verified', () => {
  ...
  assert.equal(verified.message, 'Iris wants to see what Clarity is doing.');
...
test('asks separately, and differently, for control', () => {
  ...
  assert.match(copy.message, /wants to control Clarity/);
  assert.equal(copy.allowLabel, 'Allow control');
});

/**
 * End to end over a real socket, without Electron: discovery, consent, and the
 * three questions Iris actually asks when someone says Clarity is broken.
 */
test('answers Iris over the link', async (t) => {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'clarity-applink-'));
  ...
  const found = AppLinkClient.discover(pathOptions).find((entry) => entry.appId === 'com.clarity.overlay');
  assert.ok(found, 'Clarity did not announce itself');
  ...
  assert.equal(bundle.app.slug, 'clarity');
