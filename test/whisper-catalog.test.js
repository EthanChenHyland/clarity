...
test('defines runtime targets for Windows, Linux, and both macOS architectures', () => {
  ...
  const macTarget = getRuntimeTarget('darwin', 'arm64');
  assert.equal(macTarget.kind, 'source');
  assert.equal(new URL(macTarget.repositoryUrl).protocol, 'https:');
  assert.match(macTarget.commit, /^[a-f0-9]{40}$/);
...
test('locates packaged and prepared development runtimes without downloading code', (context) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'clarity-runtime-locator-'));
